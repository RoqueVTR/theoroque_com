<section id="contact" class="wrapper view">
    <div class="page-wrapper">
        <div class="container-fluid" style="margin-top: 30px;">
            <div class="col-md-12">
                <div class="row">
			    	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 wow fadeInUp" data-wow-duration="1s">
		    			<div class="card black-text">
							<div class="card-block">
							    <!--Header-->
							    <div class="form-header mdb-color darken-4">
							        <h3><i class="fa fa-lightbulb-o"></i> Have a project in mind?</h3>
								    <p>Let's talk about it.</p>
							    </div>

							    <!--Body-->
							    <br>

							    <div class="text-center">
							    	<div class="email">
							            <br>
							            <p>Email me at:
							                <br>
							                 <span><i class="fa fa-envelope"> </i></span> <strong>roquevtr@gmail.com</strong>
							            </p>
							        </div>
							        <div class="call">
							            <br>
							            <p>Or would you prefer to call?
							                <br>
							                <span><i class="fa fa-phone"> </i></span> +63 917 507 3906 (Globe)
							            </p>
							        </div>
							    </div>
							</div>
						</div>
					</div>
					<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 wow fadeInUp" data-wow-duration="1s">
						<h3 class="card-header mdb-color darken-4 white-text"><i class="fa fa-map-marker"></i> I am here!</h3>
						<div class="overlay hm-white-slight z-depth-2">
							<div id="map" class="img-fluid"></div>
							<div class="mask"></div>

							<div id="tr-address"></div>
						</div>
					</div>
                </div>
            </div>
        </div>
    </div>
</section>